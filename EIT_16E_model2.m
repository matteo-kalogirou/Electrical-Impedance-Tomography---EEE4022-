%
% Create a 16 electrode model of the experiental tank. System has 16
...
clear hom_img hom_data imdl fmdl stim
...
[stim, meas_sel] = mk_stim_patterns(n_elec, 1, '{ad}', '{ad}', options, I);



%% --- FWD MODEL

fmdl = mk_circ_tank(8, [], 16);
...
%% ------------------------Plotting 
figure();
plot(1:1:256,hom_data.meas,1:1:256, inh_data.meas); axis tight;
ylabel('Voltage');
...
%%
figure();
[hAx,hLine1,hLine2] = plotyy(1:1:256, [hom_data.meas inh_data.meas] ,...
                             1:1:256, hom_data.meas - inh_data.meas); axis tight
hLine1(1).Color = 'g';
hLine1(2).Color = 'b';
hLine2.Color = 'y'; 
...
imdl.hyperparameter.value = 3e-7;
...
% Reconstruct Image == 3e-5 great
imdl.hyperparameter.value = 4e-5; 
...
subplot(1,2,2); show_fem(img_b12, [0,1,0]); axis off; title('Banana at 12');

%%
figure();
imdl.hyperparameter.value = 4e-5; 
show_fem(img_b12, [0,1,0]); axis off;
