...
%   
%   32 Electrode model with adjacent current drive and measurement
%   (256-10-256)x256
...
%%
path = '/Users/matteokalogirou/Documents/MATLAB/Electrical Impedance Tomography - EEE4022/Networks/b2c_32_256_pureElem_mann/';

output_density= 256;

% Load the network
for i=1:output_density
     output_net{i} = load([path 'b2c_256_pureElem_' num2str(i)  '.mat']);
end
...
% correct for the measurement pattern being inverted
% for i =1:size(stim, 2)
...
%     inh_gb_nimg.elem_data(j) = output_net{j}.a(-1*inh_exp_gb);
    inh_c1_nimg.elem_data(j) = output_net{j}.a(hom_exp_data_300 - inh_exp_c1);
    inh_c2_nimg.elem_data(j) = output_net{j}.a(hom_exp_data_300 - inh_exp_c2);
    inh_2c1_nimg.elem_data(j) = output_net{j}.a(hom_exp_data_300 - inh_exp_2c1);
    inh_2c2_nimg.elem_data(j) = output_net{j}.a(hom_exp_data_300 - inh_exp_2c2);
    inh_gb_nimg.elem_data(j) = output_net{j}.a(hom_exp_data_300 - inh_exp_gb);
end
